...
for (j in 1:19) { # makes a prediction for each threshold value
 # cycles through all data, using excluding one player each time to be used later for the accuracy check
	for (i in 1:n) {
		...
		# performs qda
		qda_out <- qda(HOF ~., data = train)
		qda_pred <- predict(qda_out, newdata = test)	
		
		# checks whether posterior probability from QDA is higher
		# than the current threshold value, then makes prediction
		if (qda_pred$posterior[1] > thresh_seq[j]) { 
...
sink('qda_cross_val.txt')
cat("QDA: SP, G, H, AB, ASG, R\n")
